...
PouchDB uses Node.js 22 because the current HTTP adapter relies on the modern `URL.parse` static method. Its complete module distribution is built before unit tests run; HTTP integration tests are excluded because they require an external CouchDB service on port 5984.
