#! /bin/bash
sudo yum update
sudo yum install gmp-devel glibc-devel libstdc++-devel gcc-c++ binutils zlib-devel libmpc-devel mpfr-devel -y
# wget https://mirror.linux-ia64.org/gnu/gcc/releases/gcc-5.5.0/gcc-5.5.0.tar.gz -O gcc-5.5.0.tar.gz --no-check-certificate
wget https://ftp.gnu.org/gnu/gcc/gcc-5.5.0/gcc-5.5.0.tar.gz -O gcc-5.5.0.tar.gz --no-check-certificate
ls -la gcc-5.5.0.tar.gz
